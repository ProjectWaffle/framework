 ...
 */
@Named
abstract class AbstractRepository<T extends BaseEntity> implements Repository<T> {
    ...
    protected T getResult(boolean isDeleted, Predicate condition) {
        final Predicate isdeletedCondition = this.criteriaBuilder.equal(this.fromRoot.get("deleted"), isDeleted);
        if (condition != null) {
            this.criteriaQuery.where(this.criteriaBuilder.and(condition, isdeletedCondition));
        } else {
        ...
        final T result = this.entityManager.createQuery(this.criteriaQuery).getSingleResult();
        this.entityManager.clear();
        return result;
    }
    
    protected T getResult() {
        return this.getResult(false, null);
    }
    
    protected T getResult(Predicate condition) {
        return this.getResult(false, condition);
    }
